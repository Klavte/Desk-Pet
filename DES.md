...
右侧可展开聊天面板，用户输入文字与角色对话。回复由 AI 大模型生成，**人格卡驱动**。支持多人格切换，可在设置面板热插拔。
...
数据流：
  ...
  窗口监控 → window/listener → agent/active.generateActiveMessage()
           → personality.getSystemPrompt() → agent/provider
