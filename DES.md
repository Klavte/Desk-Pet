...
│  助手模式: 四级+确认UI+参数级检测【待完善】                  │
├──────────────────────────────────────────────────────────┤
│  平台桥接 (Rust) —— 窗口/系统调用/后台轮询                  │
│  bash_exec / file_read / file_write / file_list            │
│  system_info / app_open / clipboard / mcp_bridge【桩】     │
└──────────────────────────────────────────────────────────┘
```
...
│   │   ├── agent-tool.ts  # agent_spawn (NORMAL)【桩】
│   │   └── file-delete.ts # file_delete (NOWAY)
│   ├── skill/             # Skill 系统 (助手模式)
│   │   ├── loader.ts      # YAML frontmatter解析 + ToolDef转换
│   │   ├── registry.ts    # Skill 注册/查询/关键词匹配
│   │   └── runner.ts      # Skill 执行编排【桩】
│   └── mcp/               # MCP 集成 (助手模式)
│       ├── manager.ts     # MCP Manager + Mock工具【Mock】
│       ├── client.ts      # MCP Client JSON-RPC协议栈【桩】
│       ├── stdio.ts       # stdio 传输【占位】
│       └── sse.ts         # SSE 传输【占位】
│
├── safety/                # 安全控制
│   └── checker.ts         # 统一安全 + 危险模式库 (BASH/FILE patterns)
│                          # 轻量模式: SAFE放行/其余拒绝
│                          # 【待补: full-checker.ts 四级完整检查+参数检测】
│
├── context/               # ★ 上下文引擎
...
└── http_get (NORMAL)              ├── file_delete (NOWAY, 硬禁止)
                                   ├── agent_spawn (NORMAL)【桩】
                                   ├── MCP Mock工具 (4个)
                                   └── Skill 工具 (3个)

Safety: SAFE放行/其余拒绝         Safety: 四级+确认UI【待完善】
Plan: 不启用                       Plan: 复杂任务启用【关键词桩】
MCP/Skill: 不加载                  MCP/Skill: Mock加载
```
...
| 打开应用 | ❌ | ✅ |
| 剪贴板操作 | ❌ | ✅ |
| MCP Server (Mock) | ❌ | ✅ (4个Mock) |
| Skill (基础) | ❌ | ✅ (3个内置) |
| SubAgent (agent.spawn) | ❌ | ⚠️ (桩) |
| Plan 步骤 | ❌ | ⚠️ (关键词桩) |
| 完整安全确认 UI | ❌ | ⚠️ (待实现) |
...
| **agent.spawn 子代理** | ⚠️ 桩 | 返回"功能开发中"，缺 fork/team 实现 |
| **助手完整安全 (四级+确认UI)** | ⚠️ 待实现 | 缺 full-checker.ts + 确认UI组件 |

### Phase 4: MCP + Skill + Plan ⚠️ 骨架完成
...
| skills/ 目录 | ✅ | `skills/*.md` 文件已存在 |
| **Skill Runner (编排执行)** | ⚠️ 桩 | 不做真实编排，只透传参数 |
| MCP Mock 工具 (4个) | ✅ | weather/calculator/note/translate |
| **MCP Manager (真实连接)** | ⚠️ 待实现 | 当前仅 Mock，缺 stdio/SSE 连接管理 |
| **MCP Client (JSON-RPC)** | ⚠️ 桩 | connect/listTools/callTool 均为空 |
| **Rust MCP Bridge** | ⚠️ 桩 | mcp_spawn/mcp_send 返回"Phase 4实现" |
...
| 优先级 | 任务 | 说明 |
|:---:|------|------|
| P0 | 助手模式完整安全 | `full-checker.ts`（四级+参数检测+确认UI组件） |
| P1 | Skill Runner 完整编排 | 解析 steps → 内循环调用 Local/MCP 工具 → 拼结果 |
| P1 | MCP 真实连接 | Manager 管理连接生命周期 + Client JSON-RPC + Rust stdio 桥接 |
| P2 | agent.spawn 子代理 | fork/team 模式实现 |
| P2 | Plan AI模型预判 | 让模型预判任务→拆解步骤（替代关键词桩） |
| P3 | 流式输出 UI 接续 | Provider 接口已支持，UI 层 Streaming 展示 |
| P3 | MCP SSE 传输 | EventSource 连接方式 |
