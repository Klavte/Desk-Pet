...
右侧可展开聊天面板，用户输入文字与角色对话。回复由 AI 大模型生成，**人格卡驱动**。支持多人格切换。
...
当桌宠**收回（隐藏到托盘）**时，AI 主动消息通过系统通知发送，确保不会被错过。通知可在设置面板中开关。
...
角色人格通过 Markdown 文件定义，位于 `src/services/ai/characters/`。**支持 HMR 热更新**（编辑即生效）。
...
| 机制 | 说明 |
|------|------|
| **全局冷却** | 触发一次后，`cooldownSeconds`（默认5000s）内不再触发 |
| **同页面冷却** | 同一页面 `samePageCooldownSeconds`（默认7800s）内不重复触发 |
| **AI 并发锁** | 一次只能有一个 AI 请求，带 `safetyTimeoutMs`（30s）安全超时 |
...
所有音效集中在 `src/services/audio/registry.ts`，不依赖外部音频文件，用 Web Audio API `OscillatorNode` 实时合成：
